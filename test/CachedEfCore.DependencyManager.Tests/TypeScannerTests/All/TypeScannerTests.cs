...
{
    public class TypeScannerTests : TypeScannerTestBase
    {
        private static void Test<T>() 
            where T : ITypeScannerTestType
        {
            var allTypes = TypeScanner.GetAllReferencedTypes(T.TestType);

            if (!allTypes.SetEquals(T.Types))
            {
                var missing = T.Types.Except(allTypes);
                var unexpected = allTypes.Except(T.Types);
